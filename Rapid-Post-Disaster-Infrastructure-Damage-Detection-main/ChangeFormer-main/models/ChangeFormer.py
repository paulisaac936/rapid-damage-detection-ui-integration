...
class EncoderTransformer(nn.Module):
    ...
    def forward_features(self, x):
        B = x.shape[0]
        outs = []
        # Use embed_dims from class init instead of hardcoded value
        # This fixes the bug where hardcoded embed_dims shadowed the parameter
        # Check if self has embed_dims attribute (set in subclasses like EncoderTransformer_x2)
        # Otherwise use the values from patch_embed layers
        if hasattr(self, 'embed_dims'):
            embed_dims = self.embed_dims
        else:
            # Fallback: infer from patch_embed layers
            embed_dims = [
                self.patch_embed1.proj.out_channels,
                self.patch_embed2.proj.out_channels,
                self.patch_embed3.proj.out_channels,
                self.patch_embed4.proj.out_channels
            ]
